# 2020.01.29
"""
acmicpc.net/problem/5988
문제집 : 처음 프로그래밍을 접할 때 - jaehoo1
문제: 짝이 없는 경재는 매일 홀로 있다보니 홀수를 판별할 수 있는 능력이 생겼다. 
...
입력: 첫 번째 줄에 숫자의 개수 N(1 <= N <= 100)이 주어진다.
    두 번째 줄부터 N+1번째 줄에 걸쳐 홀수인지 짝수인지 확인할 정수 K (1 <= K <= 10^60)가 주어진다.
출력: N개의 줄에 걸쳐 한 줄씩 정수 K가 홀수라면 'odd'를, 짝수라면 'even'을 출력한다.
"""
N = int(input())
for i in range(N):
    K = int(input())
